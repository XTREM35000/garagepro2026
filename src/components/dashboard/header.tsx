...
export default function Header({ onOpenMobile, isMobileOpen }: { onOpenMobile?: () => void; isMobileOpen?: boolean }) {
  ...
  return (
    <header
      ...
      "
    >
      <div className="h-16 px-4 md:px-8 flex items-center justify-between max-w-full">
        {/* LEFT : LOGO */}
        <div className="flex items-center gap-4">
          {/* Left area: keep empty or place Back button if needed - hide logo on desktop */}
          <div className="hidden md:block" aria-hidden="true"></div>
        </div>

        {/* RIGHT : MOBILE BURGER + ICONS + USER */}
        <div className="flex items-center gap-3">
          {/* Mobile logo on the right: visible only on small screens */}
